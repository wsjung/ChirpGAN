...
def define_discriminator(n_blocks, input_shape=(32,75,1)):
    ...
    in_image = Input(shape=input_shape)
    # conv 1x1
    d = Conv2D(64, (3,3), padding='same', kernel_initializer='he_normal')(in_image)
    d = LeakyReLU(alpha=0.2)(d)
...
def define_discriminator(n_blocks, input_shape=(32,75,1)):
    ...
    in_image = Input(shape=input_shape)
    # conv 1x1
    d = Conv2D(128, (3,3), padding='same', kernel_initializer=init, kernel_constraint=const)(in_image)
    d = LeakyReLU(alpha=0.2)(d)
    # conv 3x3 (output block)
    d = MinibatchStdev()(d)
    d = Conv2D(128, (3,3), padding='same', kernel_initializer=init, kernel_constraint=const)(d)
...
def add_generator_block(old_model):
    ...
    print('block_end: ', flush=True, end='')
    print(block_end)
    ...
    out_image = Conv2D(1, (1,1), padding='same', kernel_initializer='he_normal')(g)
    # define model
...
def define_generator(latent_dim, n_blocks, in_dim=(32,75)):
    ...
    g  = Dense(128 * in_dim[0] * in_dim[1], kernel_initializer='he_normal')(in_latent)
...
def define_generator(latent_dim, n_blocks, in_dim=(32,75)):
    ...
    in_latent = Input(shape=(latent_dim,))
    # linear scale up to activation maps
    g  = Dense(128 * in_dim[0] * in_dim[1], kernel_initializer=init, kernel_constraint=const)(in_latent)
    g = Reshape((in_dim[0], in_dim[1], 128))(g)
    # conv 4x4, input block
    g = Conv2D(128, (3,3), padding='same', kernel_initializer=init, kernel_constraint=const)(g)
    g = PixelNormalization()(g)
...
# define models
# discriminators = define_discriminator(3, (MIN_X_DIM, MIN_Y_DIM, Z_DIM))
# spot check
# d = discriminators[0][0]
# d.summary()
# d1 = discriminators[1][0]
# d1.summary()
# d2 = discriminators[2][0]
# d2.summary()
# plot_model(d2, to_file='discriminator_plot.png', show_shapes=True, show_layer_names=True)


### GENERATORS

# define models
# generators = define_generator(100, 3, (MIN_X_DIM, MIN_Y_DIM))
# spot check
# g = generators[0][1]
# g.summary()
# g1 = generators[1][1]
...
# g2 = generators[2][1]
# g2.summary()
# plot_model(g2, to_file='generator_plot.png', show_shapes=True, show_layer_names=True)
...
# number of growth phases, e.g. 4 == [75, 150, 300, 600]
n_blocks = 4
